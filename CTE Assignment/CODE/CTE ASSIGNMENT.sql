...
use mavenmovies;
-- que 1  Identify a table in the Sakila database that violates 1NF. Explain how you would normalize it to
-- achieve 1NF
-- Ans 1 .  Actor_award table violate nf1 formation in mavenmovies database violate 1nf formation we can 
-- normalised it by updating the actor_award table , avoid multivalued (each column has only atomic values),
-- Also we can create seperate tables for the columns which contain multiple values etc 
...
-- ans 3
-- if we saw the customer table in the sakila database we get to know that the column name address_id is linked 
-- with store id and both are non key attribute  and 3nf stays that table is in 2 nf from and it ensure that all
-- the non key attribute column on the table are not related with each other (one non key attribute column 
-- related to other non key attribute column) so because of that it 
 -- violate 2 and 3 nf 
...
-- que 5 Write a query using a CTE to retrieve the distinct list of actor names and the number of films they have acted in from the  actor
