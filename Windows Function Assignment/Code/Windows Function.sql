...
SUM(film.rental_rate) AS total_amount_spent, RANK() OVER (ORDER BY SUM(film.rental_rate) DESC) AS customer_rank
...
category c ON fc.category_id = c.category_id JOIN
...
FROM (SELECT film_id, title, category, rental_count,
DENSE_RANK() OVER (PARTITION BY category ORDER BY rental_count DESC) AS rank_within_category
FROM FilmRentalCounts) RankedFilms WHERE rank_within_category <= 3
...
FROM film_category fc JOIN film f ON fc.film_id = f.film_id JOIN inventory i ON f.film_id = i.film_id
JOIN rental r ON i.inventory_id = r.inventory_id
JOIN category c ON fc.category_id = c.category_id GROUP BY fc.category_id, c.name)
SELECT category_id,category_name,rental_count,SUM(rental_count)
OVER (PARTITION BY category_id ORDER BY rental_count DESC) AS running_total FROM
...
-- ans 9
WITH FilmRentalCounts AS (SELECT fc.film_id,fc.category_id,f.title,COUNT(r.rental_id) AS rental_count,
AVG(COUNT(r.rental_id)) OVER (PARTITION BY fc.category_id) AS avg_rental_count FROM film_category fc
JOIN film f ON fc.film_id = f.film_id
JOIN inventory i ON f.film_id = i.film_id
JOIN rental r ON i.inventory_id = r.inventory_id
GROUP BY fc.film_id, fc.category_id, f.title)
SELECT film_id, category_id, title, rental_count , avg_rental_count
...
SUM(rental_rate) AS monthly_revenue,ROW_NUMBER() OVER (ORDER BY SUM(rental_rate) DESC) AS ranking
FROM rental JOIN inventory ON rental.inventory_id = inventory.inventory_id
JOIN film ON inventory.film_id = film.film_id GROUP BY DATE_FORMAT(rental_date, '%Y-%m')) RankedMonths
WHERE ranking <= 5 ORDER BY ranking;